# Wheely landing page MVP (Stitch design)

Single-page marketing site for **Wheely** automotive subscription, implemented in **React + Vite + Tailwind CSS v4**. Visual and structural source of truth: `stitch/stitch.html` (Google Stitch export), translated into reusable components—**not** a copy of production site code.
...
- **MVP brand aliases** (optional): `wheely-primary`, `wheely-secondary`, `wheely-support` (**#222222**) kept for compatibility with earlier tokens.
...
- **Page canvas:** **White** base (`body` / `main`); individual sections set their own backgrounds (e.g. dark bands, **`surface`** blocks) so copy stays readable.
- **Fleet + Why Wheely (combined):** One **`#222222`** (`wheely-support`) band: **Why Wheely?** table **~2/3**; fleet **~1/3** with **3×3** grid (**nine** logos from **`public/logos/`**, larger display). Stacks on small screens (comparison first). Anchor **`#cars`**.
...
- **Footer:** Background **`#222222`** (`wheely-support`), light text, subtle borders (`white/10`), newsletter field on a translucent dark treatment; primary-yellow **Join** button.

## Page structure (top → bottom)

1. **Header (fixed):** Glass bar, logo “Wheely”, nav (Cars, How it Works, Support), **Browse Cars** CTA, **MyWheely** — **no search**.
...
3. **Fleet + Why Wheely (`FleetWhySection`):** **`#222222`**. **2/3** table + **1/3** fleet; **3×3** logos (9 brands), ~2× prior logo size. Responsive table scroll on narrow widths.
...
- Do not paste proprietary third-party production source; Stitch HTML is the approved design reference for this repo.
